...
- `Connect-AppAnalysis` — interactive and app-only (certificate) auth against Microsoft Graph
- `Get-AppAnalysis` — reports client secret / certificate expiry for Entra ID app registrations, with `-ExpiringInDays` and `-IncludeExpired` filters
- `Export-AppAnalysisReport` — CSV/HTML export
